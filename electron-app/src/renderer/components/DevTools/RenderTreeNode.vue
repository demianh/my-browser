<template>
  <div class="render-tree-node" :class="{'render-tree-node--selected': app.selectedRenderTreeNode === node}">
    <div class="render-tree-node__label" @click="$store.commit('SET_SELECTED_RENDER_TREE_NODE', node)">
      <span class="render-tree-node__opener" @click="open = !open">
        <span v-if="node.children && node.children.length">
          <span v-if="open" class="render-tree-node__open"><i class="far fa-minus-square"></i></span>
          <span v-if="!open" class="render-tree-node__close"><i class="far fa-plus-square"></i></span>
        </span>
      </span>
